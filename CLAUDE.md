...
**コマンド（Bash等）を実行する前に、以下を必ず日本語で説明すること：**

1. **目的**: なぜこのコマンドを実行するのか
2. **内容**: 何をするコマンドなのか（実際のコマンドと、その効果）

---

## commit / push について

- **commit と push は、ユーザが明示的に指示するまで絶対に行わないこと**
- ユーザから「コミットして」「プッシュして」等の指示があった場合のみ実行する
